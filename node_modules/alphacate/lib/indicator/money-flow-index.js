...
MFI.prototype = {
	...
	_compute: function * (){
		...
		let convertToResultItem = ( typicalPrice, rawMoneyFlow, positiveMoneyFlow, negativeMoneyFlow, periodPositiveMoneyFlow = 0, periodNegativeMoneyFlow = 0 ) => {
		...
		for( let i=startIndex; i<=endIndex; i++){
			...
			if( i > startIndex ){
				...
				if( typicalPrice < prevTypicalPrice ) negativeMoneyFlow = rawMoneyFlow;
			}
			...
			if( i >= startIndex + periods ){
				...
				periodNegativeMoneyFlow = sumNegativeMoneyFlow + negativeMoneyFlow;
			}

			let resultItem = convertToResultItem( typicalPrice, rawMoneyFlow, positiveMoneyFlow, negativeMoneyFlow, periodPositiveMoneyFlow, periodNegativeMoneyFlow );
	...
	_calcPeriodMoneyFlow( collection ){
		...
		for(let i=0; i<collection.length; i++){
			...
			sumNegativeMoneyFlow += item.negativeMoneyFlow;
		}
